...
import { supabase } from "../lib/supabase"; // Sidebar app/components içindeyse bu yol doğru
...
export default function Sidebar() {
  ...
  const currentCategory = searchParams.get('category'); // Seçili kategoriyi yakala
  const router = useRouter();
  
  const [user, setUser] = useState(null);
  const [categories, setCategories] = useState([]); 
  const [showBooksMenu, setShowBooksMenu] = useState(true); // Kitaplar menüsü varsayılan açık olsun
  ...
  // Kitaplar menüsü aktif mi? (Ana sayfa veya kategori seçiliyse)
  const isBooksActive = pathname === "/";
  ...
  return (
    <aside className="w-20 md:w-64 h-full bg-white border-r border-gray-100 flex flex-col shrink-0 transition-all duration-300">
      ...
      <nav className="flex-1 px-4 space-y-2 mt-4 overflow-y-auto custom-scrollbar">
        ...
        {/* 1. KİTAPLAR MENÜSÜ (Kategoriler burada) */}
        <div>
            <button 
                onClick={() => setShowBooksMenu(!showBooksMenu)}
                className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all group ${
                    isBooksActive 
                    ? "bg-[#00537d]/5 text-[#00537d]" 
                    : "text-gray-500 hover:bg-gray-50 hover:text-[#00537d]"
                }`}
            >
                <div className="flex items-center gap-4">
                    <span className={`material-icons ${isBooksActive ? "text-[#00537d]" : "text-gray-400 group-hover:text-[#00537d]"}`}>
                        library_books
                    </span>
                    <span className="font-bold text-sm hidden md:block">Kitaplar</span>
                </div>
                <span className={`material-icons text-sm transition-transform duration-300 hidden md:block ${showBooksMenu ? 'rotate-180' : ''}`}>
                    expand_more
                </span>
            </button>

            {/* Alt Menü (Kategoriler) */}
            <div className={`overflow-hidden transition-all duration-300 ease-in-out ${showBooksMenu ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
                <div className="pl-12 pr-2 py-2 space-y-1">
                    
                    {/* TÜM KİTAPLAR SEÇENEĞİ */}
                    <Link 
                        href="/" 
                        className={`block py-2 text-xs font-bold transition-colors truncate ${
                            !currentCategory && pathname === "/" ? "text-[#00537d]" : "text-gray-400 hover:text-[#00537d]"
                        }`}
                    >
                        Tüm Kitaplar
                    </Link>
                    ...
                    {categories.length > 0 ? categories.map((cat, index) => (
                        <Link 
                            ...
                            href={`/?category=${cat}`} 
                            className={`block py-2 text-xs font-bold transition-colors truncate ${
                                currentCategory === cat ? "text-[#00537d]" : "text-gray-400 hover:text-[#00537d]"
                            }`}
                        >
                            {cat}
                        </Link>
