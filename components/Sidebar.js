  ...
  return (
    <aside className="w-20 md:w-64 h-full bg-white border-r border-gray-100 flex flex-col shrink-0 transition-all duration-300">
      ...
      <div className="p-4 border-t border-gray-100">
        {user ? (
          <Link href="/profile" className="flex flex-col md:flex-row items-center gap-3 p-2 rounded-xl hover:bg-gray-50 transition-colors group">
             ...
             <div className="hidden md:block overflow-hidden flex-1">
                <p className="text-xs font-bold text-[#202020] truncate w-full group-hover:text-[#00537d]">{user.email}</p>
                <button 
                  onClick={handleLogout} 
                  // RENGİ BURADA DEĞİŞTİRDİK
                  className="text-[10px] text-[#bf293d] font-bold uppercase tracking-widest hover:underline text-left"
                >
                  Çıkış Yap
                </button>
             </div>
